...
import com.example.myapplication10_1_63.Recycler_view
...
class profile : Fragment() {
    private var bnt_gts : Button? = null
    var PhotoURL : String = ""
    var Name : String = ""
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        bnt_gts = view.findViewById<Button>(R.id.go_to_shop)
        bnt_gts!!.setOnClickListener{
            val list_item = Recycler_view()
            val frgMng = fragmentManager
            val transaction : FragmentTransaction = frgMng!!.beginTransaction()
            transaction.replace(R.id.layout, list_item,"list_item")
            transaction.addToBackStack("list_item")
            transaction.commit()
        }
